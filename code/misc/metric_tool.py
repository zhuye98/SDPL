...
class ConfuseMatrixMeter_prog(AverageMeter):
    """Computes and stores the average and current value"""
    def __init__(self, n_class):
        super(ConfuseMatrixMeter_prog, self).__init__()
        self.n_class = n_class

    def update_cm(self, pr, gt, weight=1):
        val = get_confuse_matrix(num_classes=self.n_class, label_gts=gt, label_preds=pr)
        self.update(val, weight)
        current_score = cm2F1_prog(val)
        return current_score
...
def cm2F1_prog(confusion_matrix):
    ...
    # Accuracy
    Accuracy = (TP+TN)/(TP+FP+FN+TN + np.finfo(np.float32).eps)

    # Sensitivity, hit rate, recall, or true positive rate
    Sensitivity = TP/(TP+FN + np.finfo(np.float32).eps)
    # Specificity or true negative rate
    Specificity = TN/(TN+FP + np.finfo(np.float32).eps) 
    # Precision or positive predictive value
    Precision = TP/(TP+FP + np.finfo(np.float32).eps)
    # F1 score
    F1_score = 2 * Sensitivity * Precision / (Sensitivity + Precision + np.finfo(np.float32).eps)


    # marco average
    mF1_score = np.nanmean(F1_score)
    mAccuracy = np.nanmean(Accuracy)
    mSensitivity = np.nanmean(Sensitivity)
    mSpecificity = np.nanmean(Specificity)
    mPrecision = np.nanmean(Precision)

    metrics = {'mAccuracy': mAccuracy, 'mSensitivity': mSensitivity, 'mSpecificity': mSpecificity, 'mPrecision': mPrecision, 'mF1_score': mF1_score}
    # metric = mAccuracy
    return metrics
