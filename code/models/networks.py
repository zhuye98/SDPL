...
from models.CheXRelFormer import CheXRelFormer, CheXRelFormer_AbsDiff, CheXRelFormer_LastBLK, \
                                    CheXRelFormer_Local, CheXRelFormer_AbsDiffL, ConvolutionalDifferenceCNN, CheXRelFormer_plain
from models.SDPL import SDPL
...
def define_G(args, init_type='normal', init_gain=0.02, gpu_ids=[]):
    ...
    # Mine
    elif args.net_G == 'SDPL':
        net = SDPL()
    elif args.net_G == 'CheXRelFormer_plain':
        net = CheXRelFormer_plain(embed_dim=args.embed_dim)
    ...
    total_params = sum(p.numel() for p in net.parameters())/(1024*1024)
    print(f"Total number of parameters: {round(total_params, 3)}M")
